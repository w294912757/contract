...
<body>
	...
	<div class="mainbackground">
		<div class="mainview">
				<h3>用户权限列表</h3>
				查找用户:<input type="text" id="charactername" name="charactername"
...
var init = function(){
	...
	try {  
		...
		if(conn != null){        
			...
			String sql ="select name from role;";  //实际查询语句
			stmt = conn.createStatement();  
			rs =stmt.executeQuery(sql);
			while (rs.next()) {
				comboL.add(rs.getString(1));
			} 
				...
				<%
				  ...
				  %>
					...
					testDataList.push('<tr>'+
						'<td id="name'+num+'" value="'+data[0]+'">'+data[0]+'</td>'+
						'<td><select id="sel'+num+'"><option value="" disabled selected style="display:none;">'+data[1]+'</option>'+
...
function test(){
	...
	for(var i =1;i<=count;i++){
		var str = "name"+i;
		var tname = document.getElementById(""+str+"").innerHTML;
		name.push(tname);
	}
	for(var i=1;i<=count;i++){
		var str = "sel"+i;
